...
echo "[TEST] Delivery: Master Manifest (200 OK)..."
STATUS=$(curl -k -s -o /dev/null -w "%{http_code}" "$EDGE_URL/hls/$TOKEN/$EXPIRES/$VIDEO_ID/master.m3u8")
if [ "$STATUS" == "200" ]; then echo " PASSED"; else echo " FAILED ($STATUS)"; exit 1; fi

echo "[TEST] Delivery: Variant Playlist (200 OK)..."
STATUS=$(curl -k -s -o /dev/null -w "%{http_code}" "$EDGE_URL/hls/$TOKEN/$EXPIRES/$VIDEO_ID/0/index.m3u8")
if [ "$STATUS" == "200" ]; then echo " PASSED"; else echo " FAILED ($STATUS)"; exit 1; fi

echo "[TEST] Delivery: Video Segment (200 OK)..."
STATUS=$(curl -k -s -o /dev/null -w "%{http_code}" "$EDGE_URL/hls/$TOKEN/$EXPIRES/$VIDEO_ID/0/001.ts")
if [ "$STATUS" == "200" ]; then echo " PASSED"; else echo " FAILED ($STATUS)"; exit 1; fi

# 5. Negative Test: 410 Gone (Expired Token)
PAST_EXPIRES=$(($(date +%s) - 3600))
echo "[TEST] Enforcement: Expired Token (410 Gone)..."
STATUS=$(curl -k -s -o /dev/null -w "%{http_code}" "$EDGE_URL/hls/expired-token/$PAST_EXPIRES/$VIDEO_ID/master.m3u8")
...
STATUS=$(curl -k -s -o /dev/null -w "%{http_code}" "$EDGE_URL/hls/badtoken/$EXPIRES/$VIDEO_ID/master.m3u8")
if [ "$STATUS" == "403" ]; then echo " PASSED"; else echo " FAILED ($STATUS)"; exit 1; fi

# 7. Negative Test: Path Traversal
echo "[TEST] Enforcement: Direct Path Access (403/404)..."
STATUS=$(curl -k -s -o /dev/null -w "%{http_code}" "$EDGE_URL/hls/$VIDEO_ID/master.m3u8")
if [ "$STATUS" == "403" ] || [ "$STATUS" == "404" ]; then echo " PASSED"; else echo " FAILED ($STATUS)"; exit 1; fi
